...
>
> The unit of the Plan is an **entry**, not an "activity". Sections 3 (Flow 4), 5.8–5.10 and the
> Plan edge-case table below still use the template vocabulary and are stale on that point.
...
# 5) Text Representation Wireframes

*Desktop-first. Persistent left sidebar; frames below show the content area, with the sidebar drawn only in 5.0.*
...
│  ⏱ Timer     │            [ content area ]              │
│  ▤ History   │                                          │
│  ──────────  │  <- divider: the halves are separate     │
│  ▦ Plan      │                                          │
│              │                                          │
│              │                                          │
│  ⚙︎ Settings  │                                          │
...
┌──────────────────────────────────────────┐
│                                          │
│                ○ ○ ○ ○                   │  <- cycle: 4 blocks to long break
│                                          │
│                                          │
│                 25:00                    │  <- large, calm, low contrast
│                                          │
│                 Focus                    │
│                                          │
│           ┌───────────────┐              │
...
│           └───────────────┘              │
│                                          │
│      Last: "debug JWT refresh"           │  <- click to repeat, 1-click labeling
│                                          │
└──────────────────────────────────────────┘
```

## 5.2 Timer — focus running
...
│                                          │
│                                          │
│                 18:42                    │  <- dominant element
│                                          │
│           ●  Focus · Study               │  <- tag dot + optional label
│                                          │
│                                          │
│        ⏸ Pause          ⏹ Stop           │  <- secondary, small, muted
│                                          │
└──────────────────────────────────────────┘
```

*Note: the Pause control is conditional on the open pause-vs-abort decision. If blocks cannot be paused, this row holds only Stop.*

## 5.3 Label sheet — on completion
...
│  ┌────────────────────────────────────┐  │
│  │ debug JWT ref|                     │  │  <- autofocus, type-ahead
│  └────────────────────────────────────┘  │
│                                          │
│  Recent                                  │
│  ┌──────────────┐ ┌───────────┐          │
│  │ debug JWT re │ │ read docs │          │  <- 1-click labeling
│  └──────────────┘ └───────────┘          │
│  ┌──────────────┐                        │
│  │ write specs  │                        │
│  └──────────────┘                        │
...
│                                          │
│  ┌───────────────┐                       │
│  │     SAVE      │         Skip          │  <- skip is always available
│  └───────────────┘                       │
└──────────────────────────────────────────┘
```

## 5.4 Timer — break running
...
│                 04:12                    │
│                                          │
│              Short break                 │  <- visibly different color
│                                          │
│       Step away from the screen          │
│                                          │
│              Skip break                  │  <- text link, deliberately weak
│                                          │
└──────────────────────────────────────────┘
```

## 5.5 History — day view
...
│                                          │
│  ●━━━━━━━━━━━━━━━  Study         2h 05m  │  <- proportional bar per tag
│  ●━━━━━━━━━        Work          1h 40m  │
│  ●━━━              Admin           25m   │
│                                          │
│  ──────────────────────────────────────  │
│                                          │
│  09:00  ● debug JWT refresh        25m   │
│  09:30  ● debug JWT refresh        25m   │
│  10:00  ○ short break               5m   │
│  10:05  ● read FastAPI docs        25m   │
│  10:35  ● write specs              25m   │
│  11:00  ○ long break               15m   │
│  11:15  ● client email             25m   │
│         ⚠ aborted · 12m                  │  <- aborted blocks marked
│                                          │
│  + Add block manually                    │  <- postponed
│                                          │
└──────────────────────────────────────────┘
```

## 5.6 History — empty state
...
│  └───────────────┘                       │
│                                          │
│  Delete block                            │  <- destructive, muted, confirms
│                                          │
└──────────────────────────────────────────┘
```

## 5.8 Plan — weekly template *(phase 2)* — **SUPERSEDED by SCR-30/SCR-31**

*This undated template grid is not what gets built. See `wireframes.md` SCR-30 (week list) and
SCR-31 (day timeline), which are dated. Kept here only for the density and layout rationale.*

*No Pomodoro vocabulary anywhere on this screen. Activities are named things a person does, with a duration. Nothing is counted, expected, or compared.*

```
┌────────────────────────────────────────────────────────────────────────────┐
│  My week                                [ Template │ This week ]    + Add  │
│                                                                            │
│           MON      TUE      WED      THU      FRI      SAT      SUN        │
│  ───────────────────────────────────────────────────────────────────────   │
│  anytime │Laundry│        │       │        │       │Groceries│        │    │
│  ───────────────────────────────────────────────────────────────────────   │
│   07:00  │       │        │       │        │       │        │        │     │
│   08:00  │ Gym   │        │ Gym   │        │ Gym   │        │        │     │
│   09:00  ├───────┼────────┼───────┼────────┼───────┤        │        │     │
│   10:00  │ Deep  │ Deep   │ Deep  │ Deep   │ Deep  │        │        │     │
│   11:00  │ work  │ work   │ work  │ work   │ work  │        │        │     │
│   12:00  ├───────┼────────┼───────┼────────┼───────┤        │        │     │
│   13:00  │ Lunch │ Lunch  │ Lunch │ Lunch  │ Lunch │        │        │     │
│   14:00  │       │        │       │        │       │ Hike   │        │     │
│   15:00  │Client │ Study  │Client │ Study  │       │        │        │     │
│   16:00  │ work  │        │ work  │        │       │        │        │     │
│   17:00  ├───────┼────────┼───────┼────────┤       │        │        │     │
│   18:00  │       │Spanish │       │Spanish │       │        │        │     │
│   19:00  │       │ class  │       │ class  │       │        │        │     │
│   20:00  │Dinner │ Dinner │Dinner │ Dinner │Friends│        │ Dinner │     │
│   21:00  │       │        │       │        │       │        │        │     │
│                                                                            │
│  Click any empty slot to add an activity                                   │
└────────────────────────────────────────────────────────────────────────────┘
```

**Template vs. This week:** the default tab is *Template* — the shape of a normal week, weekdays only, no dates. *This week* shows the same grid with real dates in the headers, where a single occurrence can be moved, skipped, or added without touching the template.

## 5.9 Plan — activity editor *(phase 2)*
...
│                                          │
│  Days                                    │
│  [M] [t] [W] [t] [F] [s] [s]             │  <- multi-select; M/W/F active
│                                          │
│  Time                                    │
│  ( ) Anytime                             │
│  (•) From ┌───────┐  To ┌───────┐        │
...
│           Nothing planned yet            │
│                                          │
│    Add the things that make up a         │
│    normal week — work, gym, classes,     │
│    meals, whatever you want to see.      │
│                                          │
│         ┌───────────────┐                │
│         │ ADD ACTIVITY  │                │
│         └───────────────┘                │
...
*Note the copy: no mention of blocks, focus, or the timer. This screen may be the first thing a planner-only user ever sees.*
...
Settings groups by module, so a planner-only user can see at a glance that the Timer section is irrelevant to them.

---

## Layout notes

- ~~**Persistent left sidebar** on desktop, with Timer and History grouped above a divider and Plan below it. Narrow viewports collapse it to a top bar.~~ **Superseded:** mobile-first — a bottom tab bar (`⏱ Timer · ▤ History · ▦ Plan`) at phone width, a persistent left sidebar on desktop. See SCR-01.
- **The timer number** is the largest element in the app by a wide margin — legible from across a room.
- **Tag colors** are the only saturated color in the interface. Everything else stays neutral so the data stands out. Tags are the single visual element shared across both modules.
...
- **Secondary actions** (Pause, Stop, Skip) are visually weak on purpose — during a focus block, the correct interaction is none at all.