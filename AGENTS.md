...
**Tempo** is a self-hosted, single-user Pomodoro timer and weekly planner: two loosely coupled tools that share a vocabulary but not a codepath.
...
| Auth | Single user, email + password (bcrypt), JWT bearer tokens. Hand-rolled — **do not add an auth framework.** |
...
All routers mount at the app root. Every endpoint except `/health`, `/auth/register`, and `/auth/login` requires an `Authorization: Bearer <jwt>` header.

| Prefix | Module | Notes |
| --- | --- | --- |
| `/auth` | `app/shared/user` | `POST /register` (closes once the single user exists), `POST /login` (rate-limited per IP), `GET /me` |
...
| `LOGIN_RATE_LIMIT`, `LOGIN_RATE_WINDOW_SECONDS` | backend | Per-IP sliding window on `/auth/login` |
...
- **Single-user auth, hand-rolled.** Passwords are bcrypt-hashed; sessions are JWT bearer tokens in the `Authorization` header. Do not add an auth framework, and do not move the token into a cookie — CORS is configured with `allow_credentials=False` precisely because the token is a header.
- **Registration closes once the single user exists.** `POST /auth/register` must keep returning an error after the first account.
- `/auth/login` is rate-limited per IP (`app/core/ratelimit.py`).
