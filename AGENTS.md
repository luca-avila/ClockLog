...
Copy `.env.example` to `.env` before the first run. How interpolation, the dev
override, and the `RESEND_API_KEY` placeholder work is defined in
`docs/operations.md` — read that section before touching `.env` or any compose
...
| URL | What |
| --- | --- |
| `http://localhost:3000` | Frontend dev (`npm run dev`) |
| `http://localhost:8000` | API |
| `http://localhost:8000/docs` | OpenAPI / Swagger UI |
| `http://localhost:8000/health` | Liveness probe → `{"status": "ok"}` |
| `http://127.0.0.1:3001` | Frontend in prod (Compose host port; nginx fronts it — see `docs/operations.md` § Deployment) |
...
Daily commands (`up`, `logs`, bind-mount rebuild rule), migrations, and deploy
are defined in `docs/operations.md` — follow that runbook, do not duplicate it here.

The non-negotiable rules that stay in this file:

- **Migrations always via Alembic. Never hand-written SQL.** A PR that changes a
  model must include its migration, and every autogenerated migration gets reviewed
  before commit (autogenerate misses server defaults, index renames, and column
  type changes with data implications).
- Rebuild the backend image only when `pyproject.toml` or the `Dockerfile` changes
  (`docker compose up -d --build backend`); plain `.py` edits reload via bind mount.
...
- `TEST_DATABASE_URL` is pinned by `docker-compose.override.yml` in dev; if unset it is derived from `DATABASE_URL` by swapping the database name for `clocklog_test`.
...
Defined in `docs/operations.md` (prod stack, images, environment variables,
fail-fast `${VAR:?}` semantics, CI). Do not duplicate that material here.

The two facts agents act on without opening the runbook:

- `NEXT_PUBLIC_API_URL` is baked into the frontend bundle — changing it requires
  a frontend rebuild, not a restart.
- `./scripts/ci.sh` runs everything CI runs; `.github/workflows/ci.yml` is the source.
...
Symptom → cause/fix lives in `docs/operations.md` § Monitoring and diagnosis
(troubleshooting table). Start there; the entries that matter most to agents:

- Timer under-reports in a background tab → invariant 1 violated (accumulating
  counter instead of deriving from `startedAt`).
- ESLint `no-restricted-imports` → invariant 11 violated; move the shared piece
  into `shared/`.
- `test_plan_contains_no_timer_vocabulary` on a header-only change → use the
  `plan/` license-header variant (see [License headers](#license-headers)).
